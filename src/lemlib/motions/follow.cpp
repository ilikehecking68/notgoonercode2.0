...
struct Path_Motor_Speed_Command {
    uint8_t index;
    int8_t speed; //-127 to 127
};


struct Path_3_Wire_Actuation_Command {
    uint8_t index;
    bool state; //false or true
};

struct Path_Function_Call_Index {
    size_t index;
    bool reset = false;
};
...
class Waypoint : public V2Position {
    public:
        ...
        Number speed;
        std::vector<Path_Motor_Speed_Command> motor_speed_commands;
        std::vector<Path_3_Wire_Actuation_Command> three_wire_commands;
        std::vector<Path_Function_Call_Index> func_commands;
};
...
static std::vector<Waypoint> getPath(const asset& asset) {
    ...
    for (std::string line : dataLines) {
        ...
        if (line.rfind("to:", 0) == 0){
            const std::vector<std::string> pointInput = readElement(line.substr(3), ", "); // parse line
        ...
        } else if (line.rfind("motor:", 0) == 0){
           const std::vector<std::string> pointInput = readElement(line.substr(6), ", "); // parse line
            // check if the line was read correctly
            ...
            path[path.size()].motor_speed_commands.push_back({
                .index = static_cast<uint8_t>(std::stoi(pointInput[0])),
                .speed = static_cast<int8_t>(std::stoi(pointInput[1]))
            }); 
        } else if (line.rfind("digitalout:", 0) == 0){
           const std::vector<std::string> pointInput = readElement(line.substr(11), ", "); // parse line
            // check if the line was read correctly
            if (pointInput.size() != 2) {
                logHelper.error("Failed to read path file! Are you using the right format? Raw line: {}",
                                stringToHex(line));
            ...
            path[path.size()].three_wire_commands.push_back({
                .index = static_cast<uint8_t>(std::stoi(pointInput[0])),
                .state = static_cast<bool>(std::stoi(pointInput[1]))
            }); 
        } else if (line.rfind("function:", 0) == 0){
           const std::vector<std::string> pointInput = readElement(line.substr(9), ", "); // parse line
            // check if the line was read correctly
            if (pointInput.size() != 2) {
                logHelper.error("Failed to read path file! Are you using the right format? Raw line: {}",
                                stringToHex(line));
            ...
            path[path.size()].func_commands.push_back({
                .index = static_cast<uint8_t>(std::stoi(pointInput[0])),
                .reset = !static_cast<bool>(std::stoi(pointInput[1]))
            }); 
        }
...
void follow(const asset& asset, Length lookaheadDistance, Time timeout, FollowParams params, FollowSettings settings, const std::vector<Path_Motor> motors, const std::vector<Path_3_Wire_Digital_Out> three_wires, const std::vector<Path_Function> funcs) {
    ...
    while (!timer.isDone() && helper.wait()) {
        ...
        if (prev_closest_point != closestPoint){
            for (Path_Motor_Speed_Command cmd : path[closestPoint].motor_speed_commands){
                for (Path_Motor motor : motors){
                    if (cmd.index != motor.index){
                        continue;
                    }

                    motor.corrosponding.move(cmd.speed);
                }
            }

            for (Path_3_Wire_Actuation_Command cmd : path[closestPoint].three_wire_commands){
                for (Path_3_Wire_Digital_Out three_wire : three_wires){
                    if (cmd.index != three_wire.index){
                        continue;
                    }

                    three_wire.corrosponding.set_value(cmd.state);
                }
            }

            for (Path_Function_Call_Index fci : path[closestPoint].func_commands){
                for (Path_Function func : funcs){
                    if (fci.index != func.index){
                        continue;
                    }
        ...
        prev_closest_point = closestPoint;
    }
